...
class Pengawas extends MX_Controller
{
	...
	private $prefix         = 'pengawas';
	private $url            = 'pengawas';
	...
	public function select()
	{
		...
		$join 		= [
			...
			['kabupaten k', 'kabupaten_id= user_kabupaten_id', 'left']
		];
		...
		$select = 'kabupaten ,user_id, role_name, user_photo, user_status, ' . implode(',', $aCari);
	...
	public function show_add()
	{
		...
		$data['sekolah']	= $this->m_global->get(['table'=> 't_sekolah s', 'join'=> [['kabupaten k', 'k.kabupaten_id= s.kabupaten_id']]]);
	...
	public function action_add()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			for ($i=0; $i < count($_POST['sekolah_id']); $i++) { 
				$this->m_global->insert([
					'table'=> 't_user_pengawas_data', 
					'datas'=> 
							[
								'sekolah_id'=> $_POST['sekolah_id'][$i],
								'user_id'=> $res['id']
							],
				]);
			}
	...
	public function show_edit($id)
	{
		...
		$arr_conf 			= [
			'table'	=> 't_user',
			'join'	=> [['kabupaten k', 'k.kabupaten_id= user_kabupaten_id', 'left']],
			'where'	=> [strEncrypt('user_id', TRUE) => $id]
		];
		$data['record'] 	= $this->m_global->get($arr_conf)[0];

		$data['sekolah_data']	= $this->m_global->get([
				'select'=> 's.*, k.*, user_id',
				'table'=> 't_sekolah s', 
				'join'=> [['kabupaten k', 'k.kabupaten_id= s.kabupaten_id'], ['t_user_pengawas_data u', 'u.sekolah_id= s.sekolah_id', 'left']],
				'where'=>['user_id is null or user_id = '.$data['record']->user_id =>null]
			]);
	...
	public function action_edit($id)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$data_array[$this->table_prefix . 'kabupaten_id']= $post['kabupaten_id'];
			...
			$this->m_global->delete([
				'table'=> 't_user_pengawas_data', 
				'where'=> ['user_id'=> $pengawas->user_id]
			]);
