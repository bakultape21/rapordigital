			...
			<div class="row">
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select class="form-control select2" onchange="kabupaten()" name="kabupaten_id" required >
					...
					<div class="form-group">
						...
						<select class="form-control select2" required multiple name="sekolah_id[]" required >
							...
							<?php foreach($sekolah as $key): ?>
								<option value="<?= $key->sekolah_id ?>"><?= $key->sekolah_nama.' - '. $key->kabupaten; ?></option>
							<?php endforeach; ?>
						</select>
					</div>
				</div>
				...
				<div class="col-md-4">
					<div class="form-group">
						<label class="form-control-label">Verify Password : <span class="required">*</span></label>
						<input type="password" required name="confirm" class="form-control" placeholder="Verify Password">
					</div>
				</div>
				<div class="col-md-4">
					<div class="form-group">
						...
						<select id="user_role" name="role" class="form-control select2" placeholder="Role" required>
							...
							<?php foreach ($role as $key => $val): ?>
								<option value="<?php echo $val->role_id; ?>"><?php echo ($key+1).'. '.$val->role_name; ?></option>
							<?php endforeach ?>
						</select>
					</div>
				</div>
