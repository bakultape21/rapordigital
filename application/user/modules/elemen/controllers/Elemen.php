...
class Elemen extends MX_Controller
{
	...
	public function delete_elemen($rowid='')
	{
		...
		$data['elemen_data']= @$this->cart->contents();
	...
	public function show_add()
	{
		...
		$elemen_master= $this->m_global->get(['table'=> 't_master_elemen']);

		if ( empty($this->cart->contents())) {
			foreach ($elemen_master as $key => $value) {
	...
	public function simpan_elemen()
	{
		...
		if ( !empty($this->cart->contents())) {
			$guru= $this->m_global->get(['table'=> 't_guru', 'where'=> ['guru_id'=> login_data('user_id')]])[0];

			$sekolah_elemen= [
				'table'=> 't_sekolah_elemen',
				'datas'=> ['sekolah_id'=> $guru->sekolah_id,
							'sekolah_elemen_status'=> 1],
			];
			$sekolah_elemen_id= $this->m_global->insert($sekolah_elemen)['id'];
			...
			foreach ($cart as $key ) {
				...
				$insert_elemen=[
					'sekolah_elemen_id'=> $sekolah_elemen_id,
					'elemen_data'=> $key['elemen'],
					'elemen_data_deskripsi'=> $key['deskripsi']
				];
				$this->m_global->insert(['table'=> 't_sekolah_elemen_data', 'datas'=> $insert_elemen]);
	...
	public function action_add()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($id != null) {
				...
				$result['status']     = 2;
				$result['message']    = 'Gagal menambahkan Elemen with Name <strong>' . $post['elemen'] . '</strong>';
