...
class Hasil_karya extends MX_Controller
{

	private $prefix         = 'hasil_karya';
	private $url            = 'hasil_karya';
	private $table_db       = 't_kelas_belajar_hasil_karya';
		...
		foreach ($kegiatan_belajar as $key => $value) {
			...
			$html .= '<tr id="id-'.$value->id.'">
						...
						<td>
							...
							<input type="text" class="form-control" placeholder="Judul Karya" name="hasil_karya_judul[]">
							<input type="hidden" name="id[]" value="'.$value->id.'">
						</td>
						<td>
							<textarea class="form-control" name="pengamatan[]"></textarea>
						</td>
						<td>
							'.$capaian_belajar.'
						</td>
	...
	public function select()
	{
		...
		$arr_config['table'] 	= ' t_kelas_belajar_hasil_karya c';
		...
		foreach ($result as $rows) {
			$records["data"][] = array(
				...
				$rows->hasil_karya_judul,				
				$this->_button($rows->hasil_karya_id),
			);
			$i++;
		}
	...
	public function delete($id='')
	{
		...
		$delete = [
			'table' => $this->table_db,
			'where' => [strEncrypt('hasil_karya_id', TRUE) => $id]
		];
	...
	public function cetak($id= null)
	{
		...
		$data['hasil_karya']= $this->m_global->get([
			'select'=> '*',
			'table'=> 't_kelas_belajar_hasil_karya k', 
			'join'=> [
				['t_kelas_belajar_data td', 'td.siswa_kelas_id= k.siswa_kelas_id'],
				['t_kelas_belajar kb', 'kb.kelas_id= td.kelas_id']
				
			],
			'where'=> [strEncrypt('k.hasil_karya_id', TRUE) => $id]
				]
			);
		...
		$data['cp_belajar']= $this->m_global->get([
			...
			'where'=> [
						'kd.siswa_kelas_id'=> $data['hasil_karya'][0]->siswa_kelas_id, 
						'k.kegiatan_tanggal'=> $data['hasil_karya'][0]->tanggal],
			'group'=> 'cp.cp_id'
			]);
		...
		$data['guru_sekolah']= $this->m_global->get([
			...
			'where'=> ['k.kelas_id'=> $data['hasil_karya'][0]->kelas_id]])[0];

		$data['elemen']= $this->m_global->get(['table'=> 't_sekolah_elemen_data sd', 
			'join'=> [
				['t_sekolah_elemen c', 'c.sekolah_elemen_id= sd.sekolah_elemen_id'], 
				['t_sekolah s', 's.sekolah_id= c.sekolah_id']],
			'where'=> ['s.sekolah_id'=> $data['hasil_karya'][0]->sekolah_id],
		...
		$data['siswa']= $this->m_global->get([
			...
			'where'=> ['td.siswa_kelas_id'=> $data['hasil_karya'][0]->siswa_kelas_id],
			'group'=> 's.siswa_id'
			...
			if ($_FILES) {
				...
				for ($i=0; $i < count($_POST['id']); $i++) { 
					...
					if ($this->upload->do_upload('logo')) {
						...
						$result['status']     = 2;
						$result['message']    = 'Gagal Tambahkan Karya Belajar <strong>Format Foto Tidak seuai</strong> ';
						echo json_encode($result);	
						exit();
					}

					$insert= [
						'table'=> 't_kelas_belajar_hasil_karya',
						'datas'=> [
							'tanggal'=> $_POST['tanggal'][$i],
							'siswa_kelas_id'=> $_POST['siswa_kelas_id'],
							'hasil_karya_judul'=> $_POST['hasil_karya_judul'][$i],
							'pengamatan'=> $_POST['pengamatan'][$i],
							'hasil_karya'=> $photo
						]
					];
					$this->m_global->insert($insert);

				}
			}else{
					
				$result['status']     = 2;
				$result['message']    = 'Gagal Tambahkan Karya Belajar <strong>Tidak Ada Foto</strong> ';
				echo json_encode($result);	
				exit();
			}
