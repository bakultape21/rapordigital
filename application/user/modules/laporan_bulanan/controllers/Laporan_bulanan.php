...
class Laporan_bulanan extends MX_Controller
{

	private $prefix         = 'laporan_bulanan';
	private $url            = 'laporan_bulanan';
	...
	public function select()
	{
		...
		$arr_config['table'] 	= ' t_kelas_belajar_kegiatan_bulanan k';
		...
		private function _button($id, $status= null)
	{
		...
		$html = '<a href="' . base_url() . $this->prefix . '/action_laporan_bulanan/2023-08-05" target="_blank" data-original-title="cetak" class="btn btn-sm p-1 btn-sm btn-success tooltips "> Cetak Laporan</a> ';

		$html .= '<a href="' . base_url() . '/hasil_karya/index/' . strEncrypt($id) . '" data-original-title="Hasil Karya" class="btn btn-sm  btn-sm p-1 btn-icon-md ajaxify tooltips btn-primary btnAction" >Tambahkan Hasil Karya</a>  ';

		$html .= '<a href="' . base_url() . '/analisa_cp/index/' . strEncrypt($id) . '" data-original-title="Ceklist dan Analisa Capaian" class="btn btn-sm  btn-sm p-1 btn-icon-md ajaxify tooltips btn-warning btnAction" >Ceklist dan Analisa Capaian</a>  ';
	...
	public function action_laporan_bulanan($tanggal=null)
	{
		$data['title']		= ucwords(str_replace('_', ' ', $this->title));
		$data['breadcrumb'] = [$this->title => base_url() . $this->url];
		$data['url']		= base_url() . $this->url;
		...
		$data['siswa']= $this->m_global->get([
			...
			'where'=> ['k.kelas_id'=> $_POST['kelas_id'], 'sekolah_elemen_status'=> 1],
			'group'=> 's.siswa_id'
		...
		$data['nilai']= $this->m_global->get([
			...
			group by e.elemen_data_id, siswa_kelas_id) n"]);
