...
class Foto_berseri extends MX_Controller
{

	private $prefix         = 'foto_berseri';
	private $url            = 'foto_berseri';
	private $table_db       = 't_kelas_belajar_foto_berseri';
	...
	public function select()
	{
		...
		$arr_config['table'] 	= ' t_kelas_belajar_foto_berseri c';
		...
		foreach ($result as $rows) {
			$records["data"][] = array(
				...
				$rows->pengamatan,		
				$this->_button($rows->foto_berseri_id),
			);
			$i++;
		}
	...
	public function get_form(){
		...
		$kegiatan_belajar= $this->m_global->get([
			...
			'where'=> ['(kegiatan_tanggal >="'. $_POST['awal'].'" and kegiatan_tanggal <= "'.$_POST['awal'].'")'=> null, 'n.siswa_kelas_id'=> $_POST['siswa_kelas_id']],
		...
		$elemen = $this->m_global->get(['table'=> 't_sekolah_elemen_data sd', 
			...
			'where'=> ['s.sekolah_id'=> login_data('sekolah_id')],
		...
		foreach ($kegiatan_belajar as $key => $value) {
			...
			$html .= '<tr id="id-'.$value->id.'">
						...
						<td>
							<textarea class="form-control" placeholder="pengamatan" name= "pengamatan"></textarea>
							<input type="hidden" name="id" value="'.$value->id.'">
						</td>
						<td>
							<textarea class="form-control" name="umpan_balik"></textarea>
						</td>
						<td>
							'.$capaian_belajar.'
							<br>
		...
		private function _button($id, $status= null)
	{
		...
		$html = '<a href="' . base_url() . $this->prefix . '/cetak/' . strEncrypt($id) . '" target="_blank" data-original-title="Cetak" class="btn btn-sm p-1 btn-sm btn-success tooltips "> Cetak PDF</a> ';
		$html .= '<a href="' . base_url() . $this->prefix . '/cetak_html/' . strEncrypt($id) . '" target="_blank" data-original-title="Cetak HTML" class="btn btn-sm p-1 btn-sm btn-primary tooltips "> Cetak Custome</a> ';
	...
	public function delete($id='')
	{
		...
		$delete = [
			'table' => 't_kelas_belajar_foto_berseri_data',
			'where' => [strEncrypt('foto_berseri_id', TRUE) => $id]
		];
	...
	public function cetak($id= null)
	{
		...
		$data['foto_berseri']= $this->m_global->get([
			'select'=> '*',
			'table'=> 't_kelas_belajar_foto_berseri k', 
			'join'=> [
				['t_kelas_belajar_data td', 'td.siswa_kelas_id= k.siswa_kelas_id'],
				['t_kelas_belajar kb', 'kb.kelas_id= td.kelas_id']
				
			],
			'where'=> [strEncrypt('k.foto_berseri_id', TRUE) => $id]
				]
			);
		$data['foto']= $this->m_global->get([
			'table'=> 't_kelas_belajar_foto_berseri_data',
			'where'=> [strEncrypt('foto_berseri_id', TRUE)=> $id]]);
		...
		$data['cp_belajar']= $this->m_global->get([
			...
			'where'=> [
						'kd.siswa_kelas_id'=> $data['foto_berseri'][0]->siswa_kelas_id, 
						'k.kegiatan_tanggal'=> $data['foto_berseri'][0]->tanggal],
			'group'=> 'cp.cp_id'
		]);
		...
		$data['guru_sekolah']= $this->m_global->get([
			...
			'where'=> ['k.kelas_id'=> $data['foto_berseri'][0]->kelas_id]])[0];

		$data['elemen']= $this->m_global->get(['table'=> 't_sekolah_elemen_data sd', 
			'join'=> [
				['t_sekolah_elemen c', 'c.sekolah_elemen_id= sd.sekolah_elemen_id'], 
				['t_sekolah s', 's.sekolah_id= c.sekolah_id']],
			'where'=> ['s.sekolah_id'=> $data['foto_berseri'][0]->sekolah_id],
			'order'=> ['sd.sekolah_elemen_id', 'asc']
				]);
		...
		$data['siswa']= $this->m_global->get([
			...
			'where'=> ['td.siswa_kelas_id'=> $data['foto_berseri'][0]->siswa_kelas_id],
			'group'=> 's.siswa_id'
	...
	public function cetak_html($id= null)
	{
		$data['title']		= ucwords(str_replace('_', ' ', $this->title));
		$data['breadcrumb'] = [$this->title => base_url() . $this->url];
		$data['url']		= base_url() . $this->url;
	...
	public function save_data(){
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($_FILES) {
				...
				$insert= [
					'table'=> 't_kelas_belajar_foto_berseri',
					'datas'=> [
				...
				$id=  $this->m_global->insert($insert)['id'];


				if (@$_FILES['foto1'] !=null) {

						$product_photo= null;
						$_FILES['file']['name'] = $_FILES['foto1']['name'];
				        $_FILES['file']['type'] = $_FILES['foto1']['type'];
				        $_FILES['file']['tmp_name'] = $_FILES['foto1']['tmp_name'];
				       	$_FILES['file']['error'] = $_FILES['foto1']['error'];
				   		$_FILES['file']['size'] = $_FILES['foto1']['size'];
						...
						if ($this->upload->do_upload('file')) {
							...
							$insert=[
								'foto_berseri_id'=> $id,
								'dokumentasi'=> $product_photo
							];
							$this->m_global->insert(['table'=> 't_kelas_belajar_foto_berseri_data', 'datas'=> $insert]);
				...
				if (@$_FILES['foto2'] !=null) {
						...
						$product_photo= null;
						$_FILES['file']['name'] = $_FILES['foto2']['name'];
				        $_FILES['file']['type'] = $_FILES['foto2']['type'];
				        $_FILES['file']['tmp_name'] = $_FILES['foto2']['tmp_name'];
				       	$_FILES['file']['error'] = $_FILES['foto2']['error'];
				   		$_FILES['file']['size'] = $_FILES['foto2']['size'];
						$config['upload_path'] 		= $upload_path;
						$config['overwrite'] 		= TRUE;
						$config['allowed_types'] 	= 'png|gif|jpeg|jpg';
						$config['max_size']			= 500000;
				...
				if (@$_FILES['foto3'] !=null) {
						...
						$product_photo= null;
						$_FILES['file']['name'] = $_FILES['foto3']['name'];
				        $_FILES['file']['type'] = $_FILES['foto3']['type'];
				        $_FILES['file']['tmp_name'] = $_FILES['foto3']['tmp_name'];
				       	$_FILES['file']['error'] = $_FILES['foto3']['error'];
				   		$_FILES['file']['size'] = $_FILES['foto3']['size'];
						$config['upload_path'] 		= $upload_path;
						$config['overwrite'] 		= TRUE;
						$config['allowed_types'] 	= 'png|gif|jpeg|jpg';
						$config['max_size']			= 500000;
				...
				$result['status']     = 2;
				$result['message']    = 'Gagal Tambahkan Foto Berseri <strong>Tidak Ada Foto</strong> ';
				echo json_encode($result);	
				exit();
			}
				$result['status']     = 1;
	...
	public function action_laporan_harian()
	{
		...
		$data['elemen']= $this->m_global->get(['table'=> 't_sekolah_elemen_data sd', 
			'join'=> [
				['t_cp_kelas_belajar c', 'c.elemen_data_id= sd.elemen_data_id', 'left'], 
				['t_kelas_belajar_kegiatan k', 'k.cp_id= c.cp_id', 'left']],
			'where'=> ['kegiatan_tanggal is null or kegiatan_tanggal ='=> $_POST['tanggal']],
			'group'=> 'sd.elemen_data_id'
				]
			);

		$data['cp_belajar']= $this->m_global->get([
			'select'=> 'group_concat(c.cp_id) as cp_belajar_tp',
		...
		$data['kegiatan_belajar']= $this->m_global->get([
			'select'=> 'group_concat(kegiatan_belajar) as kegiatan_belajar_nilai',
			'table'=> 't_sekolah_elemen_data sd', 
			'join'=> [
				['t_cp_kelas_belajar c', 'c.elemen_data_id= sd.elemen_data_id', 'left'], 
				['t_kelas_belajar_kegiatan k', 'k.cp_id= c.cp_id', 'left']],
		...
		$data['siswa']= $this->m_global->get([
			'table'=> 't_kelas_belajar k',
			'join'=> [['t_kelas_belajar_data td', 'td.kelas_id= k.kelas_id'], ['t_siswa s', 's.siswa_id= td.siswa_id']],
			'where'=> ['k.kelas_id'=> $data['elemen'][0]->kelas_id]]);
		...
		$data['guru_sekolah']= $this->m_global->get([
			...
			'join'=> [
				...
				['t_guru g', 'g.guru_id=k.guru_id']],
			'where'=> ['k.kelas_id'=> $data['elemen'][0]->kelas_id]])[0];
